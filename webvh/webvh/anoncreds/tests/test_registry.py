...
TEST_WEBVH_SCID = ""
TEST_WEBVH_ID = "{DOMAIN}:{NAMESPACE}:{IDENTIFIER}"
TEST_WEBVH_DID = f"did:webvh:{TEST_WEBVH_SCID}:{TEST_WEBVH_ID}"
TEST_WEBVH_SCHEMA_ID = f"{TEST_WEBVH_DID}/resources/"
TEST_WEBVH_CRED_DEF_ID = f"{TEST_WEBVH_DID}/resources/"
TEST_WEBVH_REV_REG_ENTRY = f"{TEST_WEBVH_DID}/resources/"
TEST_REGISTRAR_URL = "http://localhost:8000"
TEST_RESOLVER_URL = "http://localhost:8080/1.0/identifiers/"

async def test_supported_did_regex():
    registry = DIDWebVHRegistry()

    assert registry.supported_identifiers_regex == WEBVH_DID_VALIDATE.PATTERN
    assert bool(registry.supported_identifiers_regex.match(TEST_WEBVH_DID))
    assert bool(registry.supported_identifiers_regex.match(TEST_WEBVH_SCHEMA_ID))
    assert bool(registry.supported_identifiers_regex.match(TEST_WEBVH_CRED_DEF_ID))
    assert bool(registry.supported_identifiers_regex.match(TEST_WEBVH_REV_REG_ENTRY))
...
async def test_get_schema(mock_profile, mock_resolver):
    ...
    with patch(
        "webvh.webvh.anoncreds.registry.WebVHDIDResolver", return_value=mock_resolver
    ):
        registry = DIDWebVHRegistry()
